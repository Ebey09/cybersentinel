...
| **Extraction method** | CICFlowMeter, Engelen fork, pinned commit [VERIFY], flow timeout 120 s, activity timeout 5 s | CICFlowMeter, upstream, version used by CIC in 2020 unknown [VERIFY V25], timeouts unknown. If it stays undocumented it is recorded as `not_documented`, never guessed (ADR 0010) |
| **PCAP inference** | Allowed only after the parity test passes (ADR 0006). Blocked for now | Blocked. Extractor version unknown, so parity cannot be shown. Uploaded-CSV inference is withheld too while the extractor is not pinned; the model is trained and evaluated offline on the released CSV only (ADR 0010) |
