...
| V5 | Track A label strings match `track_a_*.yaml` | `cybersentinel-ml inspect-labels --csv <file> --column Label --experiment config/experiments/track_a_binary.yaml` per file | Track A training | **VERIFIED**: `inspect-labels --experiment config/experiments/track_a_binary.yaml` on all five CSVs reported `labels in CSV but not in label_map: none`, exit code 0. The 27 observed raw labels are mapped exactly in both Track A experiments (tests in `ml/tests/test_experiments.py`) |
...
| V7 | Class counts for both datasets (papers disagree) | Output of V5/V6, recorded in `docs/DATASETS.md` with file SHA-256 | README numbers | **PARTIAL / OPEN**. Track A evidence recorded: SHA-256, row counts (2,099,976 rows in total) and per-label raw counts of the five CSVs from `inspect-labels`, in `docs/DATASETS.md` and `ml/config/datasets.yaml`. The archive SHA-256 is recorded as previously hashed by the project owner, not re-hashed in this step. Still open: Track B class counts. V7 as a whole is not verified |
